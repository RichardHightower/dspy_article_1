...
def configure_llm():
    ...
    if provider == "openai":
        ...
        llm = dspy.LM(model=f"openai/{model}", api_key=api_key, max_tokens=4096)
        ...
        llm = dspy.LM(model=f"anthropic/{model}", api_key=api_key, max_tokens=4096)
        ...
        llm = dspy.LM(
            ...
            temperature=0.0,  # For consistent test results
        )
